...
#英文
train_english="train.raw.en"
tokenizer = MosesTokenizer()
output_data="train.txt.en"
...
#中文
train_zh="train.raw.zh"
tokenizer = MosesTokenizer()
output_data="train.txt.zh"
f_out=codecs.open(output_data,"w","utf-8")
with codecs.open(train_zh,"r","utf-8") as zh:
    for zh_line in zh.readlines():
        zh_out=tokenizer.tokenize(zh_line, return_str=True,escape=False)+"\n" #escape=False 不将标点替换为html编码
        f_out.write(zh_out)
f_out.close()
